...
void init_container_store()
{
	...
	fd = open(containerfile, O_RDWR | O_SYNC); // DedupSearch | O_DIRECT
	...
	{
		fd = open(containerfile, O_RDWR | O_CREAT | O_TRUNC | O_SYNC, 0644); // DedupSearch
		//if (!(fp = fopen(containerfile, "w+")))
		if (fd == -1)
		{ // DedupSearch
			perror(
...
void write_container(struct container *c)
{
	...
	if (destor.simulation_level < SIMULATION_APPEND)
	{
		...
		// if (fseek(fp, c->meta.id * CONTAINER_SIZE + CONTAINER_POOL_META, SEEK_SET) != 0)
...
struct container *retrieve_container_by_id(containerid id)
{
	...
	else
	{
		...
		// fseek(fp, id * CONTAINER_SIZE + CONTAINER_POOL_META, SEEK_SET);
		lseek(fd, id * CONTAINER_SIZE + CONTAINER_POOL_META, SEEK_SET);

		//printf("2: %d\n", ftell(fp));
		//printf("%d\n", fread(c->data, CONTAINER_SIZE, 1, fp)); // 4194304ll 8184
...
struct chunk *get_chunk_in_container(struct container *c, fingerprint *fp)
{
	...
	ck->size = me->len;
	ck->id = c->meta.id;
	memcpy(ck->fp, fp, sizeof(fingerprint)); // DedupSearch bug both were with &, removed it.
